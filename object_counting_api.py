...
def object_counting(input_video, detection_graph, category_index, is_color_recognition_enabled):
        ...
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        output_movie = cv2.VideoWriter('__the_output.avi', fourcc, fps, (width, height))
        ...
        with detection_graph.as_default():
          with tf.Session(graph=detection_graph) as sess:
            ...
            while(cap.isOpened()):
                ...
                counting_frame += 1
                # CUSTOM : INPUT RESIZE
                scale_percent_800 = int(80000 / input_frame.shape[1])
                scale_percent_1200 = int(120000 / input_frame.shape[1])
                
                input_frame_resized1 = cv2.resize(input_frame, (800, int(input_frame.shape[0] * scale_percent_800 / 100)), interpolation = cv2.INTER_AREA)
                input_frame_resized2 = cv2.resize(input_frame, (1200, int(input_frame.shape[0] * scale_percent_1200 / 100)), interpolation = cv2.INTER_AREA)
               
                # CUSTOM : INPUT RESIZE/
                
                # Expand dimensions since the model expects images to have shape: [1, None, None, 3]
                
                image_np_expanded = np.expand_dims(input_frame_resized2, axis=0)
                ...
                counter, csv_line, counting_mode = vis_util.visualize_boxes_and_labels_on_image_array(cap.get(1),
                                                                                                      ...
                                                                                                      use_normalized_coordinates=True,
                                                                                                      line_thickness=4)
                image_np_expanded = np.expand_dims(input_frame_resized1, axis=0)
                ...
                output_movie.write(input_frame)
                print ("writing frame", counting_frame)
                #cv2.imshow('object counting',input_frame)
